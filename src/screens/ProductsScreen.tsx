...
import { ReplaceProductModal } from '../components/roadmap/ReplaceProductModal';
...
/**
 * ProductsScreen — shopping list and meal prep tips as a navigation modal screen
 */
export const ProductsScreen: React.FC = () => {
  ...
  // Logic flags
  const shouldAnimateExit = useRef(true);
  const isOpenRef = useRef(false);

  // Local state
  const [replacedProducts, setReplacedProducts] = useState<
    Record<string, Record<number, { name: string; amount: string }>>
  >({});
  const [replaceModalVisible, setReplaceModalVisible] = useState(false);
  const [selectedReplacement, setSelectedReplacement] = useState<ReplacementSelection | null>(null);

  // Disable native swipe gesture to prevent conflict with custom animation
  ...
  // Custom PanResponder for swipe-back gesture
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (_, gestureState) => {
        // Only capture horizontal swipes starting from edge or general right swipe
        return gestureState.dx > 10 && Math.abs(gestureState.dy) < 50;
      },
      onPanResponderMove: (_, gestureState) => {
        const newX = Math.max(0, gestureState.dx); // Only allow dragging right
        translateX.setValue(newX);
      },
      onPanResponderRelease: (_, gestureState) => {
        if (gestureState.dx > SCREEN_WIDTH * 0.3 || gestureState.vx > 0.5) {
          // Swipe success - animate out
          shouldAnimateExit.current = false; // Disable auto-animation in beforeRemove
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('beforeRemove', (e: any) => {
      // If we are already animating out via swipe, skip this logic
      if (!shouldAnimateExit.current) {
        return;
      }
      ...
      Animated.timing(translateX, {
        ...
        // After animation, actually navigate back
        navigation.dispatch(e.data.action);
        // Reset X slightly later (though component unmounts usually)
      });
    });
  ...
  useFocusEffect(
    useCallback(() => {
      if (!isOpenRef.current) {
        ...
        shouldAnimateExit.current = true; // Reset exit flag
        fadeAnim.setValue(1);
  ...
  const handleGoBack = () => {
    // Standard back button press - let beforeRemove handle animation
    navigation.goBack();
  };
        ...
        product={
          selectedReplacement
            ? {
              ...
              index: selectedReplacement.index,
              originalProduct: selectedReplacement.originalProduct,
            }
            : null
        }
        onSelectProduct={(newProduct) => {
          if (selectedReplacement) {
            setReplacedProducts((prev) => ({
              ...prev,
              [selectedReplacement.category]: {
                ...prev[selectedReplacement.category],
    ...
    <View style={styles.productsCategory}>
      ...
      {products.map((product, index) => (
        <TouchableOpacity
          ...
          activeOpacity={0.8}
          onPress={() => onProductPress(product, index)}
        >
